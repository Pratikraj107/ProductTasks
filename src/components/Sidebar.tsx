...
export default function Sidebar({ currentPage, onNavigate, isCollapsed = false, onExternalNavigate }: SidebarProps) {
  ...
  const menuItems = [
    ...
    { id: 'tasks', label: 'Roadmap', icon: <CheckSquare className="w-5 h-5" /> },
    { id: 'interview', label: 'Mock Interview', icon: <MessageSquare className="w-5 h-5" /> },
  ...
  return (
    ...
    `}>
      ...
      {!isCollapsed && (
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto hide-scrollbar-desktop min-h-0">
          {/* <button
            onClick={toggleTheme}
